...
if [ "${WOKWI_PROJECT_ID}" == "" ]; then
    wokwi-server --chip esp32s3 target/${ESP_ARCH}/${BUILD_MODE}/esp32_s3_with_ili9341
else
    wokwi-server --chip esp32s3 --id ${WOKWI_PROJECT_ID} target/${ESP_ARCH}/${BUILD_MODE}/esp32_s3_with_ili9341
fi
